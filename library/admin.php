...
function bones_rss_dashboard_widget() {
	if ( function_exists( 'fetch_feed' ) ) {
		...
		$feed = fetch_feed( 'https://inspiredology.com/feed/' );        // specify the source feed
		if (is_wp_error($feed)) {
			$limit = 0;
			$items = 0;
		} else {
...
function bones_custom_dashboard_widgets() {
	//wp_add_dashboard_widget( 'bones_rss_dashboard_widget', __( 'Inspiredology Blog', 'bonestheme' ), 'bones_rss_dashboard_widget' );
}
